...
class Towers(nn.Module):
  ...
  def __init__(self, num_classes = 3):
    ...
    self.shared = nn.Linear(1024, num_classes)
  ...
  def forward(self, img_embedding, text_embedding): #  takes the image and text embedding as input
    ...
    img_f = self.img_features(img_embedding) # finds image features
    ...
    return self.shared(self.batchnorm(torch.cat([img_f, text_f], dim = 1))), img_f, text_f 
